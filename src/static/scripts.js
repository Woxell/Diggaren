const baseURL = 'https://srotify.se/';
...
async function getCurrentSongJSON(channelID) {
    const endpoint = "/radio/" + channelID;
    const options = {
        method: "GET",
        headers: {
            "Accept": "application/json"
...
async function search(phrase) {
    ...
    const endpoint = `/result?title=${encodeURIComponent(phrase.title)}&artist=${encodeURIComponent(phrase.artist)}`;
    const options = {
        method: "GET",
        headers: {
            "Accept": "application/json"
